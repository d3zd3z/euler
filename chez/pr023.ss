...
(import
  ...
  (rnrs hashtables)
  (sieve)
  (only (srfi :1 lists) filter iota reduce any))
...
(define (euler-23)
  (let* ((limit 28123)
         ...
         (abundant-set (make-eqv-hashtable)))
    (define (as-sum? elt)
      (any (lambda (x)
             (hashtable-contains? abundant-set (- elt x)))
           abundant-list))
    (display "for-each") (newline)
    (for-each (lambda (elt)
                (hashtable-set! abundant-set elt #t))
              abundant-list)
    (display "reducing") (newline)
    (reduce + 0
