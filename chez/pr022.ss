...
(define (read-names)
  (define (reader inp)
    ...
    (define (open-quote result)
      ...
      (cond ((eof-object? ch)
             (error 'open-quote "Empty input"))
            ((char=? ch #\")
             (name result '()))
            (else (error 'open-quote "Illegal character"))))

    (define (name result buf)
      (define ch (read-char inp))
      (cond ((eof-object? ch)
             (error 'name "Unexpected EOF"))
            ((char=? ch #\")
             (comma (cons (list->string (reverse buf)) result)))
            ((char-upper-case? ch)
             (name result (cons ch buf)))
            (else (error 'name "Illegal character"))))
    ...
    (define (comma result)
      ...
      (cond ((eof-object? ch)
            ...
            ((char=? ch #\,)
             (open-quote result))
            (else (error 'comma "Illegal character"))))
...
(define (euler-22)
  (define names (list-sort string<? (read-names)))
