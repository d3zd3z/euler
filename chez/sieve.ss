...
(library (sieve)
  (export
    make-sieve prime? next-prime
    divisor-count factorize
    divisors proper-divisor-sum)
  (import
    (rnrs base)
    (rnrs io simple)
    (only (chezscheme) when sort!)
    (only (srfi :1 lists) reduce)
    (srfi :9 records))
  ...
  (define (divisor-count sieve n)
	...
	(let iloop ((divide-count 0)
		    (n n))
	  (if (zero? (mod n prime))
	    (iloop (+ divide-count 1)
		   (div n prime))
	    (loop (* result (+ divide-count 1))
		  n
