...
readTriangle :: FilePath -> IO [[Int]]
readTriangle name = do
   triangle <- readFile name
   return $ reverse $ map (map read . words) $ lines $ triangle

reduce :: [[Int]] -> [Int]
reduce [] = error "Nothing to reduce"
reduce [a] = a
reduce (a:b:rs) = reduce ((select a b) : rs)

select :: [Int] -> [Int] -> [Int]
select _ [] = []
select (a1:a2:as) (b:bs) =
   (a1 `max` a2) + b : select (a2:as) bs
select _ _ = error "List got shorter"
