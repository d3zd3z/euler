...
flipNum = undigits . digits

main :: IO ()
main = print $ length $ filter (== 51) $ map (length . take 51 . chain) [1..9999]

-- Returns the flipChain, as long as the numbers are not palindromic.
chain :: Integer -> [Integer]
chain = takeWhile (\x -> x /= flipNum x) . flipChain

-- Note that this does not include the original number.
flipChain :: Integer -> [Integer]
flipChain n = next : flipChain next
   where next = n + flipNum n

undigits :: [Int] -> Integer
-- undigits [] = 0
-- undigits (n:r) = fromIntegral n + (10 * undigits r)
undigits = un 0
   where
      un x [] = x
      un x (n:r) = un (x * 10 + fromIntegral n) r

-- Compute digits of a number, but in reverse.
digits :: Integer -> [Int]
digits 0 = []
digits n = fromIntegral (n `mod` 10) : digits (n `div` 10)
