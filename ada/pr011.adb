...
procedure Pr011 is
   ...
   subtype Coord is Positive range 1 .. 20;
   type Box is array (Coord, Coord) of Natural;
   ...
   function Product (Row, Col : Coord; Del : Delta_Pair) return Natural;

   function Product (Row, Col : Coord; Del : Delta_Pair) return Natural is
      Result : Natural := 1;
      Tmp_Row : Coord := Row;
      Tmp_Col : Coord := Col;
      Tmp : Integer;
   begin
      for I in 1 .. 3 loop
         Result := Result * Source (Tmp_Row, Tmp_Col);

         Tmp := Tmp_Row + Del.Dy;
         if Tmp not in Coord then
            return 0;
         end if;
         Tmp_Row := Tmp;
   ...
   Deltas : constant array (Positive range <>) of Delta_Pair :=
     ((0, 1), (1, 0), (1, 1), (1, -1));
...
begin
   for Row in Coord loop
      for Col in Coord loop
         for Del in Deltas'Range loop
            Temp := Product (Row, Col, Deltas (Del));
            if Temp > Max then
               Max := Temp;
            end if;
         end loop;
